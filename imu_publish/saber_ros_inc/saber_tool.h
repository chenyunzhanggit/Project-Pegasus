...
typedef struct
{
    ...
    u8 dataLen;
    u8 reserve;
    GNSSNAVPVTDataStruct GnssNavPvtData;
}SaberData_GNSS_HandleType;
...
typedef struct
{
	...
	SaberData_GNSS_HandleType GnssData;
    SaberData_Baro_HandleType BaroData;
    SaberData_Distance_HandleType distance;
}SaberData;

extern int SaberAlign(unsigned char nFD, unsigned char * buffer);
...
extern int  SaberFillFrameHead(unsigned char * tmpBuf, unsigned char len);
extern u8 SaberGetGnssmod(unsigned char nFD);
#endif //SABER_TOOL_H
