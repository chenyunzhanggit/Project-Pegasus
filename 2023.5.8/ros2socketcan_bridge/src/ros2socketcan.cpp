...
void ros2socketcan::Init(const char* can_socket)
{
    ...
    publisher_ 		= this->create_publisher<can_msgs::msg::Frame>("can_receive", 100);//can_receive 指的是从下位机受到的信息，通过publisher发布出去
 //   test_pub_ 		= this->create_publisher<can_msgs::msg::Frame>("test", 100);
    subscription_ 	= this->create_subscription<can_msgs::msg::Frame>("can_send", 100 ,std::bind(&ros2socketcan::CanPublisher, this, _1));// can_send 是指将上位机计算的数据发布给下位机
...
void ros2socketcan::CanPublisher(const can_msgs::msg::Frame::SharedPtr msg) // subscriber 的回调函数
{
